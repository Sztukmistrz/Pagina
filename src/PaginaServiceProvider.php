...
class PaginaServiceProvider extends ServiceProvider
{
    ...
    */
    ...
    public function boot(\Illuminate\Routing\Router $router)
    {
        ...
        $router->middleware('localizationRedirect', \Mcamara\LaravelLocalization\Middleware\LaravelLocalizationRedirectFilter::class);

        $router->middleware('localeSessionRedirect', \Mcamara\LaravelLocalization\Middleware\LocaleSessionRedirect::class);


        $this->registerRoutes();

        
        // Using class based composers... layouter::backend.*
        view()->composer(
            'layouter::backend.*', 'Sztukmistrz\Pagina\ViewComposers\HelpTextComposer'
        );
    }
    ...
    public function register()
    {
        ...
        $this->app->singleton('Pagina', function ($app) {
            return new \Page\Manager\Manager(EntityManagerInterface);
        });
    }
    ...
    private function publishAssets($folderAssets=false)
    {
        if ($folderAssets) {
            ...
            foreach ($folderAssets as $tag => $folderAsset) {
                $this->publishes([
                    $this->packagePath.'/public/'.$folderAsset => public_path($this->publishFolderName),
                ], $tag);
            }
        }
    }
